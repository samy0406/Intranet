...
export default function StorageExtensionPage() {
  ...
  const hasInput =
    Object.values(base).some(v => v !== '') ||
    items.some(item => item.itemCode || item.lotNo || item.expiryDate)
  ...
  const handleSubmit = async (e: FormEvent<HTMLFormElement>) => {
    ...
    if (!valid) {
      ...
      setTimeout(() => {
        ...
        const el = firstBase
          ? document.getElementById(`field-${firstBase}`)
          : document.getElementById(`field-item-0`)
    ...
    try {
      ...
      else { setBaseErrors({ name: json.message }); setStatus('idle') }
    } catch {
          ...
          <form onSubmit={handleSubmit} className="space-y-5">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div id="field-department" tabIndex={-1} className="scroll-mt-6">
                ...
                <input type="text" name="department" value={base.department}
                  onChange={handleBaseChange} placeholder="例：包装課" className={inputClass} />
              ...
              <div id="field-name" tabIndex={-1} className="scroll-mt-6">
                ...
                <input type="text" name="name" value={base.name}
                  onChange={handleBaseChange} placeholder="山田 太郎" className={inputClass} />
              ...
              <div className="space-y-3">
                {items.map((item, index) => (
                  <div
                    ...
                    id={`field-item-${index}`}
                    tabIndex={-1}
                    className="bg-slate-750 border border-slate-600 rounded-xl p-4 scroll-mt-6"
