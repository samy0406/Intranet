...
export default function JudgmentCancelPage() {
  ...
  const hasInput =
    Object.values(form).some(v => v !== '') || screenshot !== null
  ...
  const handleSubmit = async (e: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      else { setErrors({ lotNo: json.message }); setStatus('idle') }
    } catch {
          ...
          <form onSubmit={handleSubmit} className="space-y-5">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div id="field-department" tabIndex={-1} className="scroll-mt-6">
                <label className={labelClass}>部署 <span className="text-rose-400">*</span></label>
                <input type="text" name="department" value={form.department}
                  onChange={handleChange} placeholder="例：包装課" className={inputClass} />
              ...
              <div id="field-name" tabIndex={-1} className="scroll-mt-6">
                ...
                <input type="text" name="name" value={form.name}
                  onChange={handleChange} placeholder="山田 太郎" className={inputClass} />
            ...
            */}
              ...
              >
                {screenshotUrl ? (
                  <div>
                    ...
                    <img src={screenshotUrl} alt="スクリーンショット"
                      className="max-h-48 mx-auto rounded-lg shadow-sm" />
                    <button type="button" onClick={clearScreenshot}
                      className="mt-3 text-xs text-slate-400 hover:text-rose-400 transition-colors">
                      ✕ 削除する
                    </button>
                  </div>
                ) : (
                  <div>
                    <p className="text-slate-400 text-sm">
                      ここをクリックして{' '}
                      <kbd className="bg-slate-700 px-1.5 py-0.5 rounded text-xs font-mono text-slate-300">Ctrl</kbd>
                      {' '}+{' '}
                      <kbd className="bg-slate-700 px-1.5 py-0.5 rounded text-xs font-mono text-slate-300">V</kbd>
                      {' '}で貼り付け
                    </p>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div id="field-itemCode" tabIndex={-1} className="scroll-mt-6">
                <label className={labelClass}>品目コード <span className="text-rose-400">*</span></label>
                <input type="text" name="itemCode" value={form.itemCode}
                  onChange={handleChange} placeholder="例：ITM-001"
                  className={`${inputClass} font-mono`} />
                {errors.itemCode && <p className={errClass}>⚠ {errors.itemCode}</p>}
              </div>
              <div id="field-lotNo" tabIndex={-1} className="scroll-mt-6">
                <label className={labelClass}>ロットNO <span className="text-rose-400">*</span></label>
                <input type="text" name="lotNo" value={form.lotNo}
                  onChange={handleChange} placeholder="例：LOT-2024001"
                  className={`${inputClass} font-mono`} />
