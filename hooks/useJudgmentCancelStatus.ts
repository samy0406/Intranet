// hooks\useJudgmentCancelStatus.ts
import { useState, ChangeEvent, FormEvent } from "react";

// ── 型定義 ──────────────────────────────────────────
export type SearchForm = { itemCode: string; lotNo: string };
export type MatchedItem = { itemCode: string; lotNo: string; shiken: string; judgmentDate: string };
export type SearchErrors = { itemCode?: string; lotNo?: string };
export type Status = "idle" | "loading";

export function useJudgmentCancelStatus() {
